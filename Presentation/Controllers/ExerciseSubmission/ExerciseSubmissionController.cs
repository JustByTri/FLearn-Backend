...
namespace Presentation.Controllers.ExerciseSubmission
{
    ...
    public class ExerciseSubmissionController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetMyExerciseSubmission(Guid? courseId, Guid? lessonId, string? status)
        {
            ...
            var result = await _progressTrackingService.GetMySubmissionsAsync(userId, courseId, lessonId, status);
        ...
        public async Task<IActionResult> GetSubmissionHistory(Guid exerciseId)
        {
            var userIdClaim = User.FindFirstValue("user_id")
                     ?? User.FindFirstValue(ClaimTypes.NameIdentifier);
            ...
            var result = await _progressTrackingService.GetExerciseSubmissionsHistoryAsync(userId, exerciseId);
            return StatusCode(result.Code, result);
        }
    }
}
