...
namespace BLL.Services.AI
{
    public class GeminiService : IGeminiService
    {
        ...
        // ============ VOICE ASSESSMENT METHODS (ĐÃ SỬA) ============

        /// <summary>
        /// ✅ SỬA ĐỔI: Thêm programName và gọi API thật
        /// </summary>
        ...
        /// <summary>
        /// ✅ SỬA ĐỔI: Thêm programLevelNames
        /// </summary>
                ...
                foreach (var question in questions.Where(q => !q.IsSkipped && !string.IsNullOrEmpty(q.AudioFilePath)))
                {
                    try
                    {
                        ...
                        parts.Add(new
                        {
                            inline_data = new
                            {
                                mime_type = "audio/mp3", // (Giả định là mp3)
                                data = base64Audio
                            }
                        });
        ...
        // ============ CÁC HÀM CŨ (CONVERSATION, COURSE, TEACHER) ============
...
Text: {text}";
        ...
        /// <summary>
        /// ✅ HÀM MỚI: Tạo prompt để tạo câu hỏi
        /// </summary>
...
Tạo 4 câu hỏi đánh giá giọng nói cho {context}.
Các câu hỏi PHẢI tuân thủ 100% định dạng JSON bên dưới.
...
[
  ...
  {{
    ""questionNumber"": 2,
    ""question"": ""(Tiêu đề câu 2, ví dụ: Giới thiệu bản thân)"",
...
";
        ...
        /// <summary>
        /// ✅ HÀM MỚI: Phân tích JSON câu hỏi
        /// </summary>
        private List<VoiceAssessmentQuestion> ParseVoiceAssessmentQuestionsResponse(string response)
        {
            ...
            // Trả về fallback nếu parse lỗi
            return GetFallbackVoiceQuestionsWithVietnamese("EN", "English");
        }

        /// <summary>
        /// ✅ SỬA ĐỔI: Thêm programLevelNames vào prompt
        /// </summary>
...
Skipped: {q.IsSkipped}"));
            ...
            // Thêm thang đo (levels) vào prompt
...
2. Cung cấp điểm tổng quan (overallScore 0-100).
...
Return JSON with overallScore, pronunciation, fluency, grammar, vocabulary (each with score and feedback), strengths, areasForImprovement.";
             ...
             string languageName)
        {
            ...
            if (languageName.Contains("Chinese")) fallbackLevel = "HSK 1";
        ...
        private List<VoiceAssessmentQuestion> GetFallbackVoiceQuestionsWithVietnamese(string languageCode, string languageName)
        {
            ...
            return languageCode.ToUpper() switch
            {
                ...
                {
                    new() { QuestionNumber = 1, Question = "请读出:", PromptText = "你好 - 世界", VietnameseTranslation = "Xin chào - Thế giới", Difficulty = "HSK 1", MaxRecordingSeconds = 30 },
                    new() { QuestionNumber = 2, Question = "请介绍你自己:", PromptText = "说出你的名字和爱好。", VietnameseTranslation = "Nói tên và sở thích.", Difficulty = "HSK 2", MaxRecordingSeconds = 60 },
                    new() { QuestionNumber = 3, Question = "描述你的家乡:", PromptText = "你的家乡是什么样的?", VietnameseTranslation = "Quê bạn thế nào?", Difficulty = "HSK 3", MaxRecordingSeconds = 90 },
                    new() { QuestionNumber = 4, Question = "谈论文化:", PromptText = "你认为中国文化的特色是什么?", VietnameseTranslation = "Đặc sắc văn hóa TQ là gì?", Difficulty = "HSK 4", MaxRecordingSeconds = 120 }
                },
